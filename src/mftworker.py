    ...
    def read_mftmem(self, compt, strt=0, endp=100, mft="C:\\$MFT"):
        ...
        try:
            ...
            if byte_s:
                ...
                self.log.emit("Unable to get entry count on Mft for progress")

            rlt, std_err = read_mft_progress(cmd, csv_data, byte_s, strt, endp, show_progress, self.progress.emit)

            if rlt == 0:
                if len(csv_data.getvalue()) != 0:

                    self.progress.emit(int(endp))
                    return csv_data
