...
class GeneralWorker(Worker):
    ...
    def __init__(self, lclhome, home_dir, database, target, drive, exclDIRS, usr, email, flth):
    ...
    def run_op(self, action):
        try:
            ...
            try:
                if action == "benchmark":
                    excluded_paths = set(os.path.join(self.basedir, d) for d in self.exclDIRS)
                    rlt = perform_read_test(self.basedir, excluded_paths)
                    ...
                    try:
                        conn = create_conn(self.database, self.target, self.email)
                        cur = conn.cursor()
