...
def set_stat(line, checks, file_dt, file_st, file_us, inode, log_q=None, logger=None):
    ...
    creation_time = getattr(file_st, "st_birthtime", file_st.st_ctime)
    c_time = epoch_to_date(creation_time)
...
def normalize_to_us(mod_time):
    """ slice ns to us """
    s = str(mod_time).strip()
    return int(s[:-3] or "0")
...
def calculate_checksum(file_path, mtime, mod_time, inode, size_int, prev_hash=None, algo="md5", st=None, retry=1, max_retry=None, cacheable=True, log_q=None, logger=None):
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            while chunk := f.read(1024 * 1024):  # 8192
        ...
        if retry > 0:
            ...
            # if not total_size:
            #     emit_log("DEBUG", f"calculate_checksum Size was zero: {file_path} checksum {checks} and total_size {total_size} filesize", log_q, logger=logger)
            ...
            if re_st == "Nosuchfile":
                ...
                mtime = epoch_to_date(re_st.st_mtime)
                return calculate_checksum(file_path, mtime, a_mod, a_ino, a_size, checks, algo, re_st, retry - 1, max_retry, cacheable, log_q, logger=logger)
