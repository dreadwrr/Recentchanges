...
def process_mft(line, checksum, filetype, search_start_dt, CACHE_F):
    ...
    if mode_attribs not in ("None", None):
        ...
        emit_log("DEBUG", f"Mft missing mode_atribs stating file : {file_path}", logs.WORKER_LOG_Q)
        try:
            ...
            emit_log("DEBUG", f"Permission denied could not stat mft entry : {line}", logs.WORKER_LOG_Q)
            return None, log_entries
        except OSError as e:
            emit_log("DEBUG", f"Error stating mft Skipping entry : {line} {type(e).__name__} err: {e}", logs.WORKER_LOG_Q)
            return None, log_entries
    ...
    if not mode:
        ...
        emit_log("DEBUG", f"missing mode_attribs line: {line}", logs.WORKER_LOG_Q)
    if mtime is None:
